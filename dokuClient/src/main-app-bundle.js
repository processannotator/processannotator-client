...
Polymer({
	...
	attached: function () {
		...
		localInfoDB.changes({ live: true, since: 'now' }).on('change', info => {
	...
	switchProjectDB: function (newProject) {
		...
		localProjectDB.changes({ live: true, since: 'now', include_docs: true }).on('change', info => {
	...
	addAnnotation: function ({ detail: { description = '', position = { x: 0, y: 0, z: 0 }, cameraPosition = { x: 0, y: 0, z: 0 }, cameraRotation = { x: 0, y: 0, z: 0 }, cameraUp = { x: 0, y: 0, z: 0 }, polygon = [] } }) {
		...
		return localProjectDB.put(annotation).then(result => {}).catch(err => {
			console.log(err);
		});
	},
	...
	loadPreferences: function () {
		...
		return localInfoDB.get('_local/lastSession').then(preferences => {
	...
	setNewProfile: function ({ prename, surname, email, color }) {
		...
		}).then(response => userDB.login(name, password)).then(response => {
	...
	setNewProject: function ({ projectname, topicname, file, emails }) {
		...
		function annotationWithCreatorProfile(doc) {
			console.log('doc');
			if (doc.creator === this.activeProfile.name) return localCachedUserDB.get(doc.creator).then(profile => {
		...
		userDB.getUser(this.activeProfile.name).then(response => {
			...
			return userDB.putUser(this.activeProfile.name, { metadata: { projects: response.projects } });
		}).then(() => {
			this.projectOpened = true;
	...
	updateElements: function (options) {
		...
		if (options.updateFile && options.updateFile === true) {
			localProjectDB.get('info').then(doc => localProjectDB.getAttachment(doc.activeTopic, 'file')).then(blob => {
